...
| 张量 | 形状 | 含义 |
|---|---|---|
| \(\mathbf{Q}\) | \(L_q \times d_k\) | 每个查询位置一行 |
| \(\mathbf{K}\) | \(L_{kv} \times d_k\) | 每个被查位置一行 |
| \(\mathbf{Q}\mathbf{K}^\top\) | \(L_q \times L_{kv}\) | **注意力矩阵**：每个查询对每个被查位置一个分数 |
| softmax 后 | \(L_q \times L_{kv}\) | 每行和为 1 |
| \(\mathbf{V}\) | \(L_{kv} \times d_v\) | 每个被查位置携带的内容 |
| 输出 | \(L_q \times d_v\) | 每个查询位置拿回一个混合后的内容向量 |

注意 \(L_q\) 与 \(L_{kv}\) **可以不相等**——这一条是第 3 节 cross-attention 的全部基础。
...
| 长度约束 | \(L_q=L_{kv}\) | \(L_q,L_{kv}\) 可完全不同 |
