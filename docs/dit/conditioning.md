...
??? note "展开：正弦位置编码到底在做什么，为什么不直接喂标量 t"

    公式与 Transformer 的位置编码同一个（DDPM/ADM 直接搬的）。设编码维度 \(d\)（常取 128 或 256，再由 MLP 升到 \(4\times\) 通道数）：

    \[
    \text{emb}(t)_{2i}=\sin\!\left(\frac{t}{10000^{2i/d}}\right),\qquad
    \text{emb}(t)_{2i+1}=\cos\!\left(\frac{t}{10000^{2i/d}}\right),\quad i=0,\dots,d/2-1
    \]

    即取 \(d/2\) 个从高到低的频率 \(\omega_i=10000^{-2i/d}\)，每个频率给一对 \((\sin\omega_i t,\ \cos\omega_i t)\)。几何上，\(t\) 沿一条曲线在 \(\mathbb{R}^d\) 中移动，每两维是一个以角速度 \(\omega_i\) 匀速旋转的单位圆——**像一组快慢不同的钟表指针**：秒针（高频）分辨相邻的 \(t\) 与 \(t{+}1\)，时针（低频）标记"大致处在扩散过程的哪个阶段"，所有读数合起来唯一确定 \(t\)。思想同二进制编码，只是把 0/1 换成了连续的三角函数。

    **为什么不直接把标量 \(t\) 喂进线性层。** \(\gamma,\beta\) 是 \(\mathbf{t}_{\text{emb}}\) 的线性函数，若输入只有一个标量，网络对 \(t\) 的依赖近似只能是低阶多项式；而真实需求高度非线性——\(\bar\alpha_t\) 在两端变化极缓、中间陡降，\(t=10\) 与 \(t=30\) 该做的事差别巨大，\(t=800\) 与 \(t=820\) 却几乎一样。MLP 又天然存在 **spectral bias**（偏向拟合低频函数），单标量输入很难学出这种局部剧变。

    关键在于 \(\{\sin\omega_i t,\cos\omega_i t\}\) 是一组 **Fourier 基**：把 \(t\) 展开到这组基上之后，后面一个线性层就能表达关于 \(t\) 的（带宽内）任意光滑函数。等于把"学一个复杂的一维函数"这件难事，换成了零参数的特征工程。NeRF 的 positional encoding、Fourier features 用的是同一个道理。

    **为什么不用 one-hot / learned embedding table**（\(t\) 只有 1000 个取值，看似可以查表）：

    | | one-hot(1000 维) / 查找表 | 正弦编码 |
    |---|---|---|
    | 相邻性 | \(t=500\) 与 \(t=501\) 完全正交，**没有"接近"的概念**，每个 \(t\) 的参数独立学 | 内积随 \(\lvert\Delta t\rvert\) 平滑衰减，相近的 \(t\) 天然共享统计强度 |
    | 连续 \(t\) | 只能整数索引 | \(t\) 取任意实数皆可 |
    | 参数量 | \(1000\times d\) | **零参数** |

    第二行在实践上是硬需求：DDIM 跳步采样、continuous-time diffusion / flow matching 中 \(t\in[0,1]\) 是连续量、SD 训练 1000 步而推理 50 步，都要求编码对任意 \(t\) 有定义。

    所以升维的目的不是让各维互斥地代表某个 \(t\)，而是**在保持"相近的 \(t\) 编码也相近"的前提下，把一个标量摊成 Fourier 基，让下游廉价地表达对 \(t\) 的复杂依赖**。
