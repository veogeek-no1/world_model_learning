...
#### 先补一个接缝：从 token 序列到单个向量 \(\mathbf{I}/\mathbf{T}\)

上面"编码成向量 \(\mathbf{I}\)"这句跳了步，值得钉清楚——否则很容易卡在"前面明明说 ViT 输出的是一串 token，这里怎么冒出一个向量"。有三个关键事实：

**① self-attention 是长度守恒的，不会自己收成一个 token。** \(N\) 个 token 进、\(N\) 个 token 出，每个位置各自拿回一个 \(D\) 维向量（回看 [Transformer 篇](../foundations/transformer.md) §2.1 那张形状表：输出是 \(L_{\mathrm{q}}\times d_v\)，"每个查询位置拿回一个向量"）。所以过完整个 encoder，你手里是 \(N{+}1\) 个向量（\(N\) 个 patch token + 1 个 [CLS]），**不是一个**。

**② "收成一个向量"是 encoder 之后单独的一步——pooling。** 要么**取 [CLS]** 那个位置的输出（训练把 loss 挂在它上面，attention 被逼着把全图信息汇聚进去，见 §2.2），要么对所有 patch token 做 **mean pooling**。这一步不属于 attention 机制，而属于任务头，所以基础篇没细讲。

**③ pooling 得到的 \(D\) 维向量还不是 \(\mathbf{I}\)——中间隔着一个投影头。** \(\mathbf{I},\mathbf{T}\) 活在一个**共享 embedding 空间**（维度 \(d_e\)），由各自塔后面一个线性层投影过去：

\[
\mathbf{I} = \mathbf{W}_{\text{img}}\,\text{pool}(\text{图像 encoder 输出}),\qquad
\mathbf{T} = \mathbf{W}_{\text{txt}}\,\text{pool}(\text{文本 encoder 输出})
\]

为什么不能省掉这个投影、直接拿 \(D\) 维向量当 \(\mathbf{I},\mathbf{T}\)？因为**两个塔的 \(D\) 通常不相等**（图像塔如 ViT-L 的 \(D_{\text{img}}=1024\)，文本塔 \(D_{\text{txt}}=512\)），维度都对不上，谈何点积。投影头把两边**分别映射到同一个 \(d_e\)**（CLIP 里常是 512 或 768），维度这才对齐。

所以记住这个区分：**token 的维度 \(D\) 是 encoder 的 hidden dim；\(\mathbf{I},\mathbf{T}\) 的维度是 \(d_e\)，来自投影头，两者不必相等。** 完整链条（图像塔为例）：

\[
\text{图像}\ \xrightarrow{\text{patchify}}\ N\ \text{个 token}\ \xrightarrow[\text{长度不变}]{\text{encoder}}\ (N{+}1)\ \text{个}\,D\,\text{维向量}\ \xrightarrow[\text{取 CLS/mean}]{\text{pool}}\ 1\ \text{个}\,D\,\text{维}\ \xrightarrow[\mathbf{W}_{\text{img}}]{\text{投影}}\ \mathbf{I}\in\mathbb{R}^{d_e}
\]

文本塔同构，终点是同一个 \(d_e\) 空间里的 \(\mathbf{T}\)。到这里两个模态才真正"同维、可比"，下面的相似度矩阵才算得出来。
