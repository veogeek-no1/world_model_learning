...
\[
\mathcal{L} = \tfrac12\Big(\underbrace{\text{CE}_{\text{行}}(\text{logits})}_{\text{每张图选对它的文}} + \underbrace{\text{CE}_{\text{列}}(\text{logits})}_{\text{每段文选对它的图}}\Big),\qquad \text{logits}_{ij}=\frac{\mathbf{I}_i\cdot\mathbf{T}_j}{\tau\,\lVert\mathbf{I}_i\rVert\,\lVert\mathbf{T}_j\rVert}
\]

即对相似度矩阵**按行、按列各做一次 softmax 交叉熵**（那个 \(\tau\) 又是温度，见 [Transformer 篇](../foundations/transformer.md) 2.3 的展开注）。
